...
exports.command = (bot, message, args) => {
    ...
    const filter = m => m.author.id == message.author.id && bot.user.id !== message.author.id;
...
collector.on('collect', m => {
    ...
    if(cpt > 0) {
        ...
        tries--
        emb.embedMaker(Discord, message, '#FF0000', `Le board : ${md.encadrer(cur_res.join(" | "))}\n\nErreurs permises restantes : ${md.encadrer(tries)}\n\n--`)
...
collector.on('end', collected => {
    ...
    if(cur_res.join("") == chosen_word.join("")){
        emb.embedMaker(Discord, message, '#00FF00',  `GG ! Tu as trouver le mot qui était ${md.encadrer(chosen_word.join(""))} en ${nb_essais} essais.`)
        collector.stop()
    }else if (tries == 0){
        emb.embedMaker(Discord, message, '#FF0000', `Bien essayer, mais tu n'as pas su trouver le mot qui était : ${md.encadrer(chosen_word.join(""))}. Une prochaine fois peut-être !`)
           
    }else if(tab.includes(chosen_word.join(""))) {
        return emb.embedMaker(Discord, message, '#00FF00',  `Ah ouais, t'es chaud toi, tu trouves tout direct, gg, le mot était ${md.encadrer(chosen_word.join(""))} et tu l'as trouver en ${nb_essais} essais.`)
    }
    
    
    else{
        return emb.embedMaker(Discord, message, '#FF0000', `Fin du temps imparti, tu n'as pas su trouver le mot qui était : ${md.encadrer(chosen_word.join(""))}. Une prochaine fois peut-être !`)
